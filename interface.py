...
class GameScreen(AbstractScreen):
    ...
    def draw(self, surface):
        ...
        msg_door = self.game_logic.get_current_door_message()
        if msg_door:
            font = self.game.font_small
            text = font.render(msg_door, True, (0, 0, 0))
            surface.blit(text, (self.PANEL_START_X + 20, 740))
        
        msg_move = self.game_logic.move_message
        if msg_move:
            txt = self.game.font_small.render(msg_move, True, (0,0,0))
            surface.blit(txt, (self.PANEL_START_X + 20, 720))
...
class RoomChoiceScreen(AbstractScreen):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT or event.key == pygame.K_a:
                ...
                if self.game_logic.select_room(chosen_room):
                    return   # go to on_enter_room()
                else:
                    print("Pas assez de gemmes !")
            # R = reroll avec un dé
            elif event.key == pygame.K_r:
                new_rooms, msg = self.game_logic.reroll_room_choices()
                # Pas de dé → message
                if new_rooms is None:
                    self.message = msg
                    return

                # Appliquer le nouveau tirage
                self.room_options = new_rooms
                self.index = 0
                self.message = msg
                return
    ...
    def draw(self, surface):
        ...
        # --- REROLL HINT R---
        hint_reroll = self.text_font.render("Press R to reroll (cost: 1 die)", True, (150, 150, 150))
        surface.blit(hint_reroll, (self.PANEL_START_X + 20, 730))

        # --- MESSAGE after reroll ---
        if hasattr(self, "message") and self.message:
            msg_surf = self.text_font.render(self.message, True, (255, 220, 0))
            surface.blit(msg_surf, (self.PANEL_START_X + 20, 770))

        
        
class PickItemScreen(AbstractScreen):
    """
    Écran permettant au joueur de récupérer les objets trouvés dans une pièce.
    ↑ / ↓ pour choisir, SPACE pour ramasser.
    """
    def __init__(self, game_context, logic, image_manager, room):
        super().__init__(game_context)
        self.logic = logic
        self.image_manager = image_manager
        self.room = room
    ...
    def handle_input(self, event):
        if event.type == pygame.KEYDOWN:
            ...
            # SPACE = pick
                ...
                # ----------------------------------
                # Catégories de ressources → ajouter directement la valeur.
                # ----------------------------------
                self.logic.add_item_to_inventory(item)
                
                self.message = self.logic.last_effect_message

                # Supprimer cet objet de la pièce.
                self.items.pop(self.index)

                # Si tous les objets sont pris → définir **items_taken** et revenir à l’écran de jeu.
                if not self.items:
                    self.room.items = []
                    self.room.items_taken = True
    ...
    def draw(self, surface):
        ...
        # title
        title = self.font_title.render("Objects found:", True, (255, 255, 255))
        ...
        for i, item in enumerate(self.items):
            ...
            txt = self.font_item.render(f"- {item}", True, color)
            surface.blit(txt, (80, y))
            y += 40

        # instruction
        hint = self.font_item.render("↑/↓ select   SPACE pick", True, (180, 180, 180))
        surface.blit(hint, (50, 700))
        
        if hasattr(self, "message") and self.message:
            text_surf = self.font_item.render(self.message, True, (255, 255, 0))
            surface.blit(text_surf, (50, 500))   #




class PurchaseItemScreen(AbstractScreen):
    """
    Interface de la boutique : le joueur peut acheter de la nourriture.
    ...
    def handle_input(self, event):
        if event.type == pygame.KEYDOWN:
            ...
            # purchase
                ...
                price = self.logic.food_prices[name]

                # Pièces insuffisantes.
                if self.logic.inventaire.pieces_or < price:
                    self.message = "Not enough gold!"
                    return

                # Déduire l’argent + obtenir l’objet.
                self.logic.inventaire.pieces_or -= price
                self.logic.add_item_to_inventory(name)
                self.message = f"You bought: {name}"

            # quit shop
            elif event.key == pygame.K_ESCAPE:
                self.room.is_shop = False
                self.game.show_game_screen()
                return
    ...
    def draw(self, surface):
        ...
        title = self.font_title.render("Kitchen Shop", True, (255, 255, 255))
        surface.blit(title, (50, 50))
        ...
        for i, name in enumerate(self.items):
            price = self.logic.food_prices[name]
            color = (255, 255, 0) if i == self.index else (255, 255, 255)
            txt = self.font_item.render(f"- {name}   {price} gold", True, color)
            surface.blit(txt, (80, y))
            y += 40

        hint = self.font_msg.render("↑/↓ select   SPACE buy   ESC cancel", True, (150, 150, 150))
        surface.blit(hint, (50, 700))

        if self.message:
            msg = self.font_msg.render(self.message, True, (255, 200, 0))
            surface.blit(msg, (50, 650))
